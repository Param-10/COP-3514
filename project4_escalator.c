...
//according to the question, i have modified the code in the function to use the pointer arithmetic. in pointer arithmetic, *(arrival+i) is equivalent to arrival[i]
...
int active_seconds(int *arrival, int n) {
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            if (*(arrival + i) > *(arrival + j)) { //this compares the ith and the jth terms of the arrival terms 
    ...
    int start_time = *arrival;
    int end_time = (*arrival) + 10;
    for (int i = 1; i < n; i++) {
